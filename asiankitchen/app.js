...
const categories = menu.reduce(
  (categories, next) => {
    if (!categories.includes(next.category)) categories.push(next.category);
    return categories;
  },
  ['All']
);
...
let mealsArray = menu;
section.innerHTML = mealsArray
  .map(
    (meal) => `<div class="menu-items col-lg-6 col-sm-12">
                <img
                  src=${meal.img}
                  alt=${meal.title}
                  class="photo"
...
categories.forEach((category) => {
  ...
  button.addEventListener('click', () => {
    ...
    mealsArray =
      category === 'All'
        ? menu
        : menu.filter((meal) => meal.category === category);
    let mealsToDisplay = mealsArray
      .map(
        (meal) => `<div class="menu-items col-lg-6 col-sm-12">
                <img
                  src=${meal.img}
      ...
      .join('');
    section.innerHTML = mealsToDisplay;
  });
});
